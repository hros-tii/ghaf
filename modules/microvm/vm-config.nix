...
#
# Provides ghaf.virtualization.vmConfig for hypervisor selection, resource
...
# - Hypervisor selection - QEMU by default, Crosvm for AdminVM, and per-VM overrides
...
#   ├── Hypervisor selection: default plus per-system-VM overrides
  ...
  systemVmConfigType = types.submodule {
    options = {
      hypervisor = mkOption {
        ...
        default = null;
        description = ''
          Hypervisor used for this system VM.
          If null, uses ghaf.virtualization.vmConfig.defaultHypervisor.
        '';
        example = "crosvm";
      };
...
in
{
  ...
  options.ghaf.virtualization.vmConfig = {
    defaultHypervisor = mkOption {
      type = types.enum [
        "qemu"
        "crosvm"
      ];
      default = "qemu";
      description = "Default hypervisor for system VMs without a per-VM override.";
    };
      ...
      example = literalExpression ''
        {
          ...
          adminvm.hypervisor = "qemu"; # Optional AdminVM fallback
  ...
  config.ghaf.virtualization.vmConfig.sysvms.adminvm.hypervisor = lib.mkDefault "crosvm";
}
